...
import { PlusCircle, Download, LayoutGrid, List, Target, BarChart3, Search, TrendingUp, StickyNote } from "lucide-react";
...
const Dashboard = () => {
  ...
  const [view, setView] = useState<"overview" | "card" | "table" | "notes">("overview");
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {/* Application Statistics - Fixed mobile layout */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 md:gap-4">
        <Card className="border-l-4 border-l-primary">
          <CardContent className={`${isMobile ? 'p-3' : 'p-4'}`}>
            <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold`}>{stats.total}</div>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Total Programs</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-blue-500">
          <CardContent className={`${isMobile ? 'p-3' : 'p-4'}`}>
            <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-blue-600`}>{stats.considering}</div>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Researching</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-orange-500">
          <CardContent className={`${isMobile ? 'p-3' : 'p-4'}`}>
            <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-orange-600`}>{stats.applied}</div>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Applied</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-green-500">
          <CardContent className={`${isMobile ? 'p-3' : 'p-4'}`}>
            <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-green-600`}>{stats.accepted}</div>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Accepted</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-red-500 col-span-2 lg:col-span-1">
          <CardContent className={`${isMobile ? 'p-3' : 'p-4'}`}>
            <div className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-red-600`}>{stats.rejected}</div>
            <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>Rejected</p>
          </CardContent>
        </Card>
      </div>

      <Tabs value={view} onValueChange={(v) => setView(v as "overview" | "card" | "table" | "notes")}>
              ...
              <Card>
                ...
                <CardContent>
                  <div className={`grid grid-cols-1 ${isMobile ? 'gap-3' : 'md:grid-cols-2 gap-4'}`}>
                    {programs.slice(0, isMobile ? 2 : 4).map((program) => (
                  ...
                  {programs.length === 0 && (
                    <div className="text-center py-8 md:py-12 border-2 border-dashed border-gray-200 rounded-lg">
                      <Target className="h-8 w-8 md:h-12 md:w-12 text-gray-400 mx-auto mb-4" />
                      <h3 className="text-base md:text-lg font-medium text-gray-900 mb-2">Start Your Application Journey</h3>
        ...
        <TabsContent value="card" className="space-y-4 md:space-y-6">
          ...
          <div className="flex flex-col gap-4 items-start">
            ...
            <div className={`grid gap-2 w-full ${isMobile ? 'grid-cols-2' : 'md:grid-cols-4'}`}>
          ...
          {filteredPrograms.length === 0 ? (
            ...
            <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'md:grid-cols-2 xl:grid-cols-3'}`}>
