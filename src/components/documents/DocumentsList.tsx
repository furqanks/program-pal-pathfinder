...
const DocumentsList = ({
  ...
  const handleRename = async (docId: string) => {
    console.log("handleRename called with:", {
      docId,
      editName
    });
    if (editName.trim()) {
  ...
  return <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={onCreateNew || (() => onSelectDocument(null))} className="h-8 px-2">
          <PlusCircle className="h-4 w-4 mr-1" />
          New
        </Button>
      </div>
      
      {documentVersions.length === 0 ? <div className="text-sm text-muted-foreground py-4 text-center border rounded-md">
          No documents yet
        </div> : <div className="space-y-2 max-h-[40vh] md:max-h-[50vh] overflow-y-auto pr-1">
        ...
        return <div key={doc.id} className="relative group">
                <Button variant={selectedDocumentId === doc.id ? "default" : "outline"} onClick={() => onSelectDocument(doc.id)} className="w-full justify-start h-auto py-2 px-3 pr-10 bg-muted hover:bg-muted/80 text-foreground border-border">
                  <div className="flex flex-col items-start text-left">
                    <div className="flex items-center gap-1">
                      ...
                      {editingDocument === doc.id ? <Input value={editName} onChange={e => setEditName(e.target.value)} onBlur={() => handleRename(doc.id)} onKeyDown={e => {
                  ...
                  if (e.key === 'Escape') {
                    ...
                    {program && <span className="text-xs text-muted-foreground mt-1 truncate w-full">
                        {program.programName}
                      </span>}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="absolute right-1 top-1 h-8 w-8 p-0 opacity-70 group-hover:opacity-100 transition-opacity z-10" onClick={e => {
                e.stopPropagation();
