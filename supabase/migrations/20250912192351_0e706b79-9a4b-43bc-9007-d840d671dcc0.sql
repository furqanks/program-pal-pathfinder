-- Security improvements for user_sessions table
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM public.user_sessions 
      WHERE user_id = p_user_id AND is_active = true) >= 10 THEN
...
-- Step 6: Update session activity function to use hashed tokens
...
-- Step 9: Create audit trigger for session changes
...
DROP TRIGGER IF EXISTS audit_session_termination ON public.user_sessions;
CREATE TRIGGER audit_session_termination
  AFTER UPDATE ON public.user_sessions
  FOR EACH ROW EXECUTE FUNCTION public.audit_session_changes();
...
USING (
  user_id = auth.uid() AND (
    -- Only show essential data, hide sensitive hashes
    session_token_hash IS NOT NULL OR session_token IS NOT NULL
  )
);

-- Step 11: Create cleanup job reminder
COMMENT ON FUNCTION public.cleanup_expired_sessions IS 'Run this function periodically (daily) via cron job to clean expired sessions';

-- Step 12: Add session security constraints
ALTER TABLE public.user_sessions 
ADD CONSTRAINT check_session_expiry CHECK (expires_at > created_at);

-- Step 13: Create view for safe session display
